export const menuInfos = [
    {
        id: -1,
        icon: 'ep:menu',
        title: '仪表盘',
        componentName: 'DashBoard',
        parentId: 0,
        openType: 0,
        showIndex: 1,
        url: '/dashboard',
        redirect: '/dashboard/analysis',
        hidden: false
    },
    {
        id: -2,
        title: '分析页',
        url: '/dashboard/analysis',
        keepAlive: false,
        component: 'dashboard/analysis/analysis',
        componentName: 'Analysis',
        openType: 0,
        parentId: -1,
        showIndex: 1,
        hidden: false
    },
    {
        id: -3,
        title: '工作台',
        url: '/dashboard/workbench',
        keepAlive: true,
        component: 'dashboard/workbench/workbench',
        componentName: 'Workbench',
        openType: 0,
        parentId: -1,
        showIndex: 2,
        hidden: false
    },
    {
        id: 1,
        icon: 'ep:memo',
        title: '平台管理',
        componentName: '',
        parentId: 0,
        openType: 0,
        showIndex: 1,
        url: '/workbench',
        redirect: '/systemDesign/remind',
        hidden: false
    },
    {
        id: 2,
        title: '提醒设置',
        url: '/systemDesign/remind',
        keepAlive: true,
        component: 'systemDesign/remind/remind',
        componentName: 'Remind',
        openType: 0,
        parentId: 1,
        showIndex: 1,
        hidden: false
    },
    {
        id: 3,
        title: '菜单管理',
        url: '/systemManage/systemMenu',
        keepAlive: true,
        component: 'systemManage/systemMenu/systemMenu',
        componentName: 'systemMenu',
        openType: 0,
        parentId: 7,
        showIndex: 2,
        hidden: false
    },
    {
        id: 4,
        title: '系统模块按钮',
        url: '/systemDesign/systemButton',
        keepAlive: true,
        component: 'systemDesign/systemButton/systemButton',
        componentName: 'SystemButton',
        openType: 0,
        parentId: 7,
        showIndex: 2,
        hidden: false
    },
    {
        id: 9,
        title: '系统参数设置',
        url: '/systemDesign/sysParams',
        keepAlive: true,
        component: 'systemDesign/sysParams/sysParams',
        componentName: 'SysParams',
        openType: 0,
        parentId: 1,
        showIndex: 2,
        hidden: false
    },
    {
        id: 5,
        title: '异常信息',
        url: '/systemDesign/exception',
        keepAlive: true,
        component: 'systemDesign/exception/exception',
        componentName: 'Exception',
        openType: 0,
        parentId: 1,
        showIndex: 2,
        hidden: false
    },
    {
        id: 6,
        title: '系统设置',
        url: '/systemDesign/systemSet',
        keepAlive: true,
        component: 'systemDesign/systemSet/systemSet',
        componentName: 'SystemSet',
        openType: 0,
        parentId: 1,
        showIndex: 1,
        hidden: false
    },
    {
        id: 7,
        icon: 'ep:setting',
        title: '系统管理',
        componentName: '',
        parentId: 0,
        openType: 0,
        showIndex: 2,
        url: '/systemManage',
        redirect: '/systemManage/user',
        hidden: false
    },
    {
        id: 8,
        title: '用户管理',
        url: '/systemManage/user',
        keepAlive: true,
        component: 'systemManage/userInfo/user/user',
        componentName: 'User',
        openType: 0,
        parentId: 7,
        showIndex: 1,
        hidden: false
    },
    {
        id: 13,
        title: '角色',
        url: '/systemManage/role',
        keepAlive: true,
        component: 'systemManage/roleManage/role/role',
        componentName: 'Role',
        openType: 0,
        parentId: 7,
        showIndex: 7,
        hidden: false
    },
    {
        id: 14,
        title: '角色权限',
        url: '/systemManage/rolePermission',
        keepAlive: true,
        component: 'systemManage/roleManage/rolePermission/rolePermission',
        componentName: 'RoleAuthority',
        openType: 0,
        parentId: 7,
        showIndex: 1,
        hidden: false
    },
    {
        id: 15,
        title: '字典管理',
        url: '/systemManage/dictionary',
        keepAlive: true,
        component: 'systemManage/dictionary/dictionary',
        componentName: 'Dictionary',
        openType: 0,
        parentId: 7,
        showIndex: 1,
        hidden: false
    },
    {
        id: 12,
        icon: 'ep:location',
        title: '测试',
        keepAlive: true,
        componentName: 'Test',
        parentId: 0,
        openType: 0,
        showIndex: 4,
        component: 'test/test',
        url: '/test',
        redirect: '',
        hidden: true,
        children: []
    },
    {
        id: 16,
        icon: 'ep:location',
        title: '跳转外链',
        componentName: 'OutPage',
        parentId: 0,
        openType: 1,
        showIndex: 5,
        component: '',
        url: '',
        outPageUrl: 'https://www.baidu.com',
        redirect: '',
        hidden: false,
        children: []
    }
]
